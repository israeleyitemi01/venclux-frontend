...
import { ArrowUpRight, ArrowDownRight, Calendar, Download, Loader2 } from "lucide-react";
  ...
  useEffect(() => {
    const fetchAnalytics = async () => {
      try {
        ...
        console.error("Error connecting frontend matrix to analytics endpoint:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading || !analyticsData) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px] gap-2">
        ...
        <p className="text-sm text-slate-500 font-medium">Assembling live operational intelligence metrics...</p>
      </div>
    );
  }

  const { cards, revenueTimeline, categoryData, ordersPerDay, topProducts } = analyticsData;
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        <MetricCard 
          label="Revenue" 
          value={`₦${cards.revenue >= 1000000 ? `${(cards.revenue / 1000000).toFixed(2)}M` : cards.revenue.toLocaleString()}`} 
          sub="vs last period" 
          trend={cards.revenueTrend} 
          color="emerald" 
        />
        <MetricCard label="Orders" value={cards.orders.toString()} sub="vs last period" trend={cards.ordersTrend} color="blue" />
        <MetricCard label="Customers" value={cards.customers.toString()} sub="new this period" trend={cards.customersTrend} color="amber" />
        <MetricCard label="Avg Conversion" value={`${cards.conversionRate}%`} sub="conversion trend" trend={cards.conversionTrend} color="rose" />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm lg:col-span-2 space-y-4">
          ...
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={revenueTimeline} margin={{ top: 5, right: 10, left: -10, bottom: 0 }}>
                <defs>
                  <linearGradient id="aGrad" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#10B981" stopOpacity={0.25} />
                    <stop offset="95%" stopColor="#10B981" stopOpacity={0} />
                ...
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#E2E8F0" />
                <XAxis dataKey="date" tickLine={false} axisLine={false} stroke="#94A3B8" tick={{ fontSize: 11 }} />
                <YAxis tickLine={false} axisLine={false} stroke="#94A3B8" tick={{ fontSize: 11 }} tickFormatter={(v) => `₦${v >= 1000 ? `${(v / 1000).toFixed(0)}k` : v}`} />
                <Tooltip formatter={(v) => [`₦${v.toLocaleString()}`, "Revenue"]} />
                <Area type="monotone" dataKey="value" stroke="#10B981" strokeWidth={3} fill="url(#aGrad)" />
              </AreaChart>
            </ResponsiveContainer>
          ...
          <div className="h-52">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={ordersPerDay} margin={{ top: 5, right: 10, left: -20, bottom: 0 }}>
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#E2E8F0" />
                <XAxis dataKey="day" tickLine={false} axisLine={false} stroke="#94A3B8" tick={{ fontSize: 11 }} />
                <YAxis tickLine={false} axisLine={false} stroke="#94A3B8" tick={{ fontSize: 11 }} allowDecimals={false} />
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm space-y-4">
          ...
          <div className="space-y-4">
            {topProducts.length === 0 ? (
              <p className="text-xs text-slate-400 text-center py-8">No settled transactions tracked during this timeframe.</p>
            ) : (
              topProducts.map((p) => (
                <div key={p.name}>
                  <div className="flex items-center justify-between text-sm mb-1">
                    <span className="text-slate-700 font-medium truncate max-w-[70%]">{p.name}</span>
